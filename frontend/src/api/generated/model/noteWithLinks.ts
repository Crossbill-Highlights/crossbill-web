/**
 * Generated by orval v8.22.0 🍺
 * Do not edit manually.
 * crossbill API
 * OpenAPI spec version: 0.1.0
 */
import type { Flashcard } from './flashcard.ts';
import type { NoteLinkedChapter } from './noteLinkedChapter.ts';
import type { NoteLinkedHighlight } from './noteLinkedHighlight.ts';
import type { NoteLinkedTag } from './noteLinkedTag.ts';

/**
 * Note response with linked entity summaries.
 */
export interface NoteWithLinks {
  id: number;
  user_id: number;
  title: string;
  body: string;
  kind: string | null;
  book_ids: number[];
  chapter_ids: number[];
  highlight_ids: number[];
  tag_ids: number[];
  created_at: string;
  updated_at: string;
  chapters?: NoteLinkedChapter[];
  highlights?: NoteLinkedHighlight[];
  tags?: NoteLinkedTag[];
  flashcards?: Flashcard[];
}
